      ...
      img.onload = () => {
        ...
        if (showLogo && logo) {
          ...
          logoImg.onload = () => {
            ...
            if (showLocation && location) {
              ...
              ctx.fillText(
                dateStr,
                margin + logoSize + 20,
                canvas.height - margin - 10,
              );
              ctx.font = "14px Arial, sans-serif";
              ctx.fillText(
                timeStr,
                margin + logoSize + 20 + ctx.measureText(dateStr).width + 15,
                canvas.height - margin - 10,
              );
            }
          ...
          if (showLocation && location) {
            ...
            ctx.fillText(
              timeStr,
              margin + ctx.measureText(dateStr).width + 15,
    ...
    <div className="w-full h-full bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950">
      ...
      <main className="flex-1 flex flex-col overflow-hidden h-[calc(100vh-80px)]">
        ...
        <div className="flex-1 bg-black relative overflow-hidden">
          ...
          {/* Bottom Container - 70% info / 30% capture button */}
          {!capturedImage && !showAllControls && (
            <div className="absolute bottom-4 left-4 right-4 h-25 flex items-center z-10">
              {/* Info Panel (70%) */}
              <div className="w-8/10">
                <div className="bg-black/70 backdrop-blur-md rounded-lg p-2 border border-white/10 shadow-lg">
                  <div className="flex items-center gap-4 overflow-hidden">
                    ...
                    <div className="flex-1 min-w-0">
                      {/* Subtitle and location in single line */}
                      ...
                      {/* Date and time in second line */}
                      <div className="flex items-center gap-2 text-[10px] text-slate-300 mt-1">
                        <Calendar className="w-3 h-3 flex-shrink-0" />
                        <span>{format(currentDateTime, "MMM dd, yyyy")}</span>
                        <span className="mx-1">•</span>
                        <Clock className="w-3 h-3 flex-shrink-0" />
                        <span>{format(currentDateTime, "hh:mm:ss a")}</span>
                      </div>
                    </div>
                  </div>
                </div>
